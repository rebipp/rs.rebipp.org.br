...
let $lookupDoc := http:send-request(<http:request method='get' href='{"https://raw.githubusercontent.com/rebipp/rs.rebipp.org.br/master/term-lists/term-lists.csv"}'/>)[2]
...
<nav class="navbar navbar-expand navbar-dark bg-secondary">
    <div class="container">
        <a class="navbar-brand mr-md-3" href="http://www.rebipp.org.br">REBIPP</a>
...
<script src="https://www.rebipp.org.br/theme/js/jquery.min.js"></script>,
<script src="https://www.rebipp.org.br/theme/js/popper.min.js"></script>,
<script src="https://www.rebipp.org.br/theme/js/bootstrap.min.js"></script>,
<script src="https://www.rebipp.org.br/theme/js/theme.js"></script>
};
...
{
let $repoPath := "https://raw.githubusercontent.com/rebipp/rs.rebipp.org.br/master/"
...
declare function html:term-metadata($record as element(),$version as xs:string,$replacements as element()*,$ns as xs:string) as element()
{
   ...
   (: terms not defined by REBIPP may have different version patterns, or may not have versions :)
   if (contains($record/term_isDefinedBy/text(), "http://rs.rebipp.org.br/" ))
   then (
...
(: Generates web page for a vocabulary; usually following the pattern http://rs.rebipp.org.br/{vocab}/ :)
...
{
let $thisVersion := "http://rs.rebipp.org.br/version/"||$record/vocabulary_localName/text()||$record/vocabulary_modified/text()
...
<div>{
  ...
  if ($std != "")
  then (
    <strong>Part of REBIPP Standard: </strong>,<a href='{$std}'>{$std}</a>,<br/>
    )
  else (
    <span>Not part of any REBIPP Standard</span>,<br/>
    ),
...
(: Generates web page for a vocabulary version; usually following the pattern http://rs.rebipp.org.br/version/{vocab}/{iso-date} :)
...
(: Generates web page for the decisions list; ; following the pattern http://rs.rebipp.org.br/decisions/ :)
...
(: Generates web page for a term list; ; usually following the pattern http://rs.rebipp.org.br/{vocab}/{list}/ :)
...
(: Generates web page for term lists versions; usually following the pattern http://rs.rebipp.org.br/{vocab}/version/{term-list}/{iso-date} :)
